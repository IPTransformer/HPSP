...
from utils.evaluation.semseg_eval import SemSegEvaluator
from utils.evaluation.parsing import get_parsing
...
class ParsingEvaluator:
    ...
    def __init__(self, parsingGt=None, parsingPred=None, maskPred=None,
                 gt_dir=None, pred_dir=None, score_thresh=0.01, num_parsing=20,
                 metrics=['mIoU', 'APp', 'APr'], APr_filename={}, APh_filename={}):
        """
        ...
        self.APr_filename = APr_filename
        self.APh_filename = APh_filename
        ...
        if 'mIoU' in self.metrics or 'miou' in self.metrics:
            ...
            print('The Global Parsing Images: {}'.format(len(parsingGt)))
            self.semseg_eval = SemSegEvaluator(
                parsingGt, self.gt_dir, maskPred=self.maskPred, num_classes=self.num_parsing,
    ...
    def _prepare_APp(self):
        class_recs = dict()
        npos = 0
        image_ids = self.parsingGt.coco.getImgIds()#[:10]
        # img_name_list = self.APr_filename.keys()
    ...
    def _compute_class_apr(self, instance_par_gt_dir, instance_par_pred_dir, img_name_list, class_id):
        ...
        for img_name in tqdm(img_name_list, desc='Calculating class: {}..'.format(class_id)):
            ...
            # --------------------------------------------------------------
            instance_img_pred, pred_scores_allCls = self.APr_filename[img_name]
            mask_class = instance_img_pred==class_id
            has_class = mask_class.sum(axis=(1,2))>0
            pred_masks = mask_class[has_class]
            num_pred_instance = pred_masks.shape[0]

            # ToDo correct the scores in computing APr

            # print(len(scores), len(true_pos[0]))


            pred_scores = pred_scores_allCls[:, class_id-1]
            ...
            scores += pred_scores[has_class].tolist()
    ...
    def computeAPh(self):
        ...
        # img_name_list = []
        img_name_list = self.APh_filename.keys()
        # while len(img_name_list) == 0:
    ...
    def evaluate(self):
        print('Evaluating Parsing predictions')
...
def parsing_nms_base(pred_ins, pred_mask_brforeargmax, nms_thresh=0.5, num_parsing=20, num_instance=12, TP_THRESHOLD=0.6):
    pred_ins = pred_ins.sigmoid()
    pred_mask = pred_mask_brforeargmax.max(1)[1]

    pred_ins_binary = pred_ins > TP_THRESHOLD
    pred_mask_binary = pred_mask > 0

    TP_pixel = pred_ins_binary * pred_mask_binary  # 12 x H x W
    parsings = pred_ins_binary * pred_mask

    parsing_scores = pred_ins * TP_pixel
    instance_scores = parsing_scores.sum(dim=(1, 2)) / (TP_pixel.sum(dim=(1, 2))+1e-10)

    parsings = parsings.cpu().numpy()
    instance_scores = instance_scores.cpu().numpy()

    # Calcute raea ratio
    _, H, W = pred_ins.shape
    pred_ratio = TP_pixel.sum(dim=(1,2)) / (H*W)
    ratio_keeped = pred_ratio > 0.01

    # print(instance_scores)


    # # calculate part score
    # part_scores = np.zeros((12, 19))
    # for cls in range(1, num_parsing):
    #     TP_part_pixel = pred_mask == cls
    #     pred_ins_part = pred_ins * TP_part_pixel
    #     part_scores_cls = pred_ins_part.sum(dim=(1,2)) / (TP_part_pixel.sum(dim=(1,2))+1e-10)
    #     part_scores[:,cls-1] = part_scores_cls.cpu().numpy()
    #

    # calculate part score based semantic segmentation
    part_scores_baseSem = np.zeros((num_instance, num_parsing-1))
    pred_mask_softmax = pred_mask_brforeargmax[0].softmax(dim=0)
    pred_mask_softmax_scores, _ = pred_mask_softmax.max(dim=0)
    for cls in range(1, num_parsing):
        TP_part_pixel = pred_mask == cls
        # pred_ins_part = pred_mask_softmax_scores * TP_part_pixel
        part_tmp = TP_part_pixel * pred_ins_binary
        part_tmp_scores = part_tmp * pred_mask_softmax_scores
        part_scores_cls = part_tmp_scores.sum(dim=(1,2)) / (part_tmp.sum(dim=(1,2))+1e-10)
        part_scores_baseSem[:,cls-1] = part_scores_cls.cpu().numpy()
    part_scores = part_scores_baseSem
    ...
    def cal_one_mean_imu(image_array, label_array, _num_parsing):
        hist = fast_hist(label_array, image_array, _num_parsing).astype(np.float)
        num_cor_pix = np.diag(hist)
        num_gt_pix = hist.sum(1)
        iu = num_cor_pix / num_gt_pix
        return iu

    def parsing_iou(src, dsts, num_classes=20):
        ious = []
        for d in range(dsts.shape[0]):
            iou = cal_one_mean_imu(src, dsts[d], num_classes)
            ious.append(np.nanmean(iou))
        return ious
    ...
    sorted_parsings = parsings[sorted_ids]
    sorted_instance_scores = instance_scores[sorted_ids]
    sorted_ins_binary = pred_ins_binary[sorted_ids]
    # sorted_ins_binary = TP_pixel[sorted_ids]
    if part_scores is not None:
        sorted_part_scores = part_scores[sorted_ids]
    # keepped = [True] * sorted_instance_scores.shape[0]
    keepped = ratio_keeped[sorted_ids].tolist()
    for i in range(sorted_instance_scores.shape[0] - 1):
        # if i == 10:
        #     print('123')
        if not keepped[i]:
            continue
        # ious = parsing_iou(sorted_parsings[i], sorted_parsings[i + 1:], num_parsing)
        ious = parsing_iou(sorted_parsings[i], sorted_parsings[i + 1:], num_parsing)
        # print(ious)
        for idx, iou in enumerate(ious):
            if iou >= nms_thresh:
                keepped[i + 1 + idx] = False
    final_parsings = sorted_parsings[keepped]
    instance_scores = sorted_instance_scores[keepped]

    human_ins = sorted_ins_binary[keepped].cpu()
    # bg_ins = torch.zeros(1, H, W)
    # print(human_ins.shape)

    # human_ins = torch.cat((bg_ins, human_ins), dim=0)
    # human_ins = human_ins.max(0)[1]

    if part_scores is not None:
        part_scores = sorted_part_scores[keepped]

    return final_parsings, instance_scores, part_scores, human_ins.numpy()
...
def parsing_nms_base_mm(pred_ins, pred_mask_brforeargmax, nms_thresh=0.5, num_parsing=20, TP_THRESHOLD=0.6):
    pred_ins = pred_ins.sigmoid()
    pred_mask = pred_mask_brforeargmax.max(1)[1]
    pred_ins_binary = pred_ins>TP_THRESHOLD
    pred_mask_binary = pred_mask > 0
    ...
    _, H, W = pred_ins.shape
    pred_ratio = TP_pixel.sum(dim=(1,2)) / (H*W)
    ratio_keeped = pred_ratio > 0.0005
    ...
    # calculate part score based semantic segmentation
    part_scores_baseSem = np.zeros((12, 19))
    pred_mask_softmax = pred_mask_brforeargmax[0].softmax(dim=0)
    pred_mask_softmax_scores, _ = pred_mask_softmax.max(dim=0)
    for cls in range(1, num_parsing):
        TP_part_pixel = pred_mask == cls
