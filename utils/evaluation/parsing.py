...
def get_parsing_mhp(root_dir, file_name, parsing_ids, num_ins):
    human_dir = root_dir.replace('images', 'parsing_annos')
    # category_dir = root_dir.replace('Images', 'Category_ids')
    ...
    # human_mask = cv2.imread(human_path, 0)
    # category_mask = cv2.imread(category_path, 0)
    num = num_ins[file_name]
    parsing = []
    for i in range(num):
        tmp_name = file_name+'_'+'{:0>2d}'.format(num)+'_'+'{:0>2d}'.format(i+1)+'.png'
        anno_path = os.path.join(human_dir, tmp_name)
        # ins_mask = cv2.imread(anno_path, 0)
        ins_mask = cv2.imread(anno_path)[:,:,2]
        parsing.append(ins_mask)
        # print(file_name, anno_path)
    return parsing
