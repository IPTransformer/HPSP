...
class augmentation_scale(object):
    ...
    def __call__(self, sample):
        ...
        resized_catg = []
        for anno in catg:
            resized_anno = cv2.resize(anno, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
            resized_catg.append(resized_anno)
...
class augmentation_rotate(object):
    ...
    def __call__(self, sample):
        ...
        rotated_catg = []
        for anno in catg:
            rotated_anno = cv2.warpAffine(anno, M, dsize=(int(new_im_width), int(new_im_height)),
                                    flags=cv2.INTER_NEAREST,
                                    borderMode=cv2.BORDER_CONSTANT,
                                    borderValue=(255, 255))
            rotated_catg.append(rotated_anno)
...
class augmentation_cropped(object):
    ...
    def __call__(self, sample):
        ...
        cropped_catg = []
        for anno in catg:
            crop_anno = np.ones((self.crop_y, self.crop_x), dtype='int')*255
            crop_anno[store_start_y:store_end_y, store_start_x:store_end_x] = anno[crop_start_y:crop_end_y,
                                                                              crop_start_x:crop_end_x]

            cropped_catg.append(crop_anno)
    ...
    def __call__(self, sample):
        ...
        if doflip:
            ...
            # flip mask for arm, leg, shoe
                left_idx = [5, 7, 22, 24, 26, 28, 30, 32]
                right_idx = [6, 8, 23, 25, 27, 29, 31, 33]
                for i in range(len(left_idx)):
                    right_pos = np.where(flipped_anno == right_idx[i])
                    left_pos = np.where(flipped_anno == left_idx[i])
                    flipped_anno[right_pos[0], right_pos[1]] = left_idx[i]
                    flipped_anno[left_pos[0], left_pos[1]] = right_idx[i]

                flipped_catg.append(flipped_anno)
...
class ToOneHotTensor(object):
    def __call__(self, sample):
        ...
        catg_sum = np.stack(catg, axis=2).sum(axis=2)
        catg_255 = catg_sum>=255

        catg_rm255 = catg
        catg_rm255_ = []
        for anno in catg_rm255:
            anno[anno==255] = 0
            catg_rm255_.append(anno)


        catg_stack = np.stack(catg_rm255_, axis=2)
        catg_stack_true = catg_stack>0
        catg_conflict = catg_stack_true.sum(axis=2)>1

        catg_255 += catg_conflict
        catg_sum[catg_255] = 255


        has_human = catg_stack.sum(axis=(0,1))
        catg_stack = catg_stack[:,:, has_human>0]>0
        num_ins = sum(has_human>0)

        human_ins = torch.zeros((H, W, 18))
        human_ins[:, :, :num_ins] = torch.from_numpy(catg_stack[:,:,:18])
        human_ins = human_ins.permute(2, 0, 1)
        ...
        return {'image': img,
                'catg': torch.from_numpy(catg_sum).long(),
                'person_center': person_center,
                # 'human': human,
                'human_correct': human_ins.long(),
...
class MHPDataset(data.Dataset):
    ...
    def __init__(self, args,
                 # data_path='datas/LV-MHP-V2/',
                 data_path='/mnt/workspace/jiangzi/data/LV-MHP-v2/',
                 mode="train",
                 img_size=512):
        ...
        if self.mode == 'train':
            ...
            self.img_dir = self.data_path+'val/images'

        self.img_names = os.listdir(self.img_dir)
        self.anno_names = os.listdir(self.img_dir.replace('images', 'parsing_annos'))
    ...
    def __getitem__(self, item):
        ...
        cat_path = self.img_dir.replace('images', 'parsing_annos')
        ...
        name_to_match = pure_name+'_'
        anno_names = list(filter(lambda x: x.startswith(name_to_match), self.anno_names))

        annos = []
        for anno_name in anno_names:
            # _cat = cv2.imread(os.path.join(cat_path, anno_name))
            # _cat1 = _cat[:, :, 2]

            mask_gt = np.array(Image.open(os.path.join(cat_path, anno_name)))
            _cat = mask_gt[:,:,0] if len(mask_gt.shape)==3 else mask_gt
        ...
        sample = {'image': _img, 'catg': annos, 'person_center': person_center}
    ...
    def transform_tr(self, sample):
        composed_transforms = transforms.Compose([
            ...
            augmentation_scale(crop_size=self.img_size,scale_min=0.8, scale_max=1.2),
            # augmentation_scale(crop_size=self.img_size, scale_min=0.8, scale_max=1.2, istrain=False),
            ...
            augmentation_cropped(crop_x=self.img_size[1], crop_y=self.img_size[0], max_center_trans=20),
            augmentation_flip(flip_prob=0.5),
            # augmentation_scale(crop_size=se
            #
            #
            #
            # lf.img_size, istrain=False),
...
if __name__ == '__main__':
    ...
    args.local_rank = 0
    args.rotate=10
    dataset = MHPDataset(args, mode='train', img_size=[512, 512])
    print(len(dataset))
    ...
    # for i in range(10):
        ...
        print(i, image.shape, catg.shape, human_ins.shape)
