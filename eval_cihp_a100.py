...
from utils.evaluation.parsing_eval import ParsingEvaluator, parsing_nms_base
...
if __name__ == '__main__':
    json_file = 'datas/CIHP_val.json'
    root = 'datas/CIHP/Validation/Images/'
    seg_root = 'datas/CIHP/Validation/Category_ids/'
    ...
    parser.add_argument('--img-size', nargs='+', type=int, default=[448, 512, 640, 800],
                        help='img size to bre trained')
    parser.add_argument('--nclass', type=int, default=20,
                        help='img size to bre trained')
    parser.add_argument('--ins', type=int, default=512,
                        help='img size to bre trained')
    # checking point
    ...
    model = build_model(backbone=backbone, num_classes=20, num_instances=12,
    ...
    # for i in range(10):
        ...
        output_ins_mix = torch.zeros((1, 12, h, w), device=device)
        output_sem_mix = torch.zeros((1, 20, h, w), device=device)
        for size in ms:
            ...
            # output_ins = output_ins.cpu()
            # output_sem = output_sem.cpu()
        ...
        if args.mix_ins:
            output_ins_mix = output_ins_mix / len(ms)
        ...
        parsings, instance_scores, part_scores, human_ins = parsing_nms_base(pred_ins=output_ins_mix[0],
                                                                    ...
                                                                    num_parsing=20, num_instance=12,
                                                                      nms_thresh=NMS_THRESHOLD,
                                                                      TP_THRESHOLD=TP_THRESHOLD)


        parsings = parsings[instance_scores>0.7]
